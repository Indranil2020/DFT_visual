...
class IEFPCMToolInput(ToolInput):
    geometry: str = Field(...)
...
@register_tool
class IEFPCMTool(BaseTool[IEFPCMToolInput, ToolOutput]):
    """IEF-PCM integral equation formalism PCM."""
    name: ClassVar[str] = "calculate_iefpcm"
    description: ClassVar[str] = "Calculate energy with IEF-PCM solvation."
    category: ClassVar[ToolCategory] = ToolCategory.SOLVATION
    version: ClassVar[str] = "1.0.0"
    
    def _execute(self, input_data: IEFPCMToolInput) -> Result[ToolOutput]:
        try:
            import psi4
            psi4.core.clean()
            ...
            psi4.set_options({
                ...
                "pcm__input": f'''
                    Units = Angstrom
                    Medium {{SolverType = IEFPCM eps = {solvent_params["eps"]}}}
                '''
            })
            energy = psi4.energy(f"{input_data.method}/{input_data.basis}", molecule=mol)
            
            data = {"total_energy": float(energy), "solvent": input_data.solvent, "model": "IEF-PCM"}
            psi4.core.clean()
            return Result.success(ToolOutput(success=True, message=f"IEF-PCM: E = {energy:.10f}", data=data))
        except Exception as e:
            return Result.failure(CalculationError(code="IEFPCM_ERROR", message=str(e)))

def calculate_iefpcm(geometry: str, solvent: str = "water", **kwargs) -> ToolOutput:
    return IEFPCMTool().run({"geometry": geometry, "solvent": solvent, **kwargs})
