...
class DDCOSMOToolInput(ToolInput):
    geometry: str = Field(...)
...
@register_tool
class DDCOSMOTool(BaseTool[DDCOSMOToolInput, ToolOutput]):
    """ddCOSMO domain-decomposition COSMO solvation."""
    name: ClassVar[str] = "calculate_ddcosmo"
    description: ClassVar[str] = "Calculate energy with ddCOSMO solvation."
    category: ClassVar[ToolCategory] = ToolCategory.SOLVATION
    version: ClassVar[str] = "1.0.0"
    
    def _execute(self, input_data: DDCOSMOToolInput) -> Result[ToolOutput]:
        try:
            import psi4
            psi4.core.clean()
            ...
            psi4.set_options({
                ...
                "ddx": True,
                "ddx_solvent_epsilon": solvent_params["eps"],
            })
            energy = psi4.energy(f"{input_data.method}/{input_data.basis}", molecule=mol)
            
            data = {"total_energy": float(energy), "solvent": input_data.solvent, "model": "ddCOSMO"}
            psi4.core.clean()
            return Result.success(ToolOutput(success=True, message=f"ddCOSMO: E = {energy:.10f}", data=data))
        except Exception as e:
            return Result.failure(CalculationError(code="DDCOSMO_ERROR", message=str(e)))

def calculate_ddcosmo(geometry: str, solvent: str = "water", **kwargs) -> ToolOutput:
